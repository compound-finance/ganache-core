...
BlockchainDouble.prototype.getBlockIndex = async function (number) {
  let i = 0, u = 0;
  for (let j = 0; j < this.blockSkips.length; j++) {
    const [lower, upper] = this.blockSkips[j];
    if (number <= lower)
      return i + number - u;
    if (number <= upper)
      return i + lower - u;
    i = i + Math.max(lower - u, 0);
    u = upper;
  }
  return i + number - u;
}
...
BlockchainDouble.prototype.getBlock = async function (number, callback) {
  var self = this;
  ...
  if (number.indexOf("0x") >= 0) {
    ...
    if (hash.length > 40) {
      ...
      // Block number
      const index = await this.getBlockIndex(to.number(hash));
      return this.data.blocks.get(index, callback);
    }
  } else {
    if (number === "latest" || number === "pending") {
...
BlockchainDouble.prototype.mineBlockNumber = async function (number, callback) {
  const forkBlockNumber = to.number(this.forkBlockNumber);
  const currentBlockNumber = await this.getBlockNumber();
  this.blockNumberAdjustment = number - currentBlockNumber;
  this.blockSkips.push([currentBlockNumber - forkBlockNumber, number - forkBlockNumber]);
  this.processNextBlock(undefined, err => {
    this.blockNumberAdjustment = 0;
    callback(err, "0x0");
  });
};

BlockchainDouble.prototype.getBlockNumber = async function () {
  return new Promise(
    (okay, fail) => {
      this.data.blocks.last((err, block) => {
        if (err) {
          ...
          okay(block ? to.number(block.header.number) : 0);
        }
      });
    });
}
