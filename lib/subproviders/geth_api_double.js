...
GethApiDouble.prototype.evm_reset = function(params, callback) {
  const provider = this.state._provider;
  const options = provider._applyDefaultOptions(params || {})
  this.state = new StateManager(options, provider);
  this.state.initialize(error => error ? callback(null, {error}) : callback(null, options))
};
...
GethApiDouble.prototype.evm_mineBlockNumber = function(number, callback) {
  this.state.blockchain.mineBlockNumber(number, callback);
}
